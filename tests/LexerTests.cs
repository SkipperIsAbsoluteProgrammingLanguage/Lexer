...
namespace LexerTests
{
    ...
    {
        private readonly Lexer.Lexer.Lexer _lexer;

        public LexerTests()
        {
            // Общий лексер для всех тестов
        }
        ...
        {
            // Arrange
            var lexer = new Lexer.Lexer.Lexer("");
        ...
        public void Tokenize_IntegerLiteral_ReturnsCorrectToken(string input, TokenType expectedType, string expectedText)
        {
            ...
            // Assert
            Assert.Equal(2, result.Count); // Число + EOF
        ...
        {
            // Arrange
            var source = "fn factorial(int n) -> int";
            var lexer = new Lexer.Lexer.Lexer(source);
        ...
        {
            // Arrange
            var source = "x = 5 // это комментарий";
            var lexer = new Lexer.Lexer.Lexer(source);
        ...
        public void Tokenize_TracksPositionCorrectly()
        {
            ...
            Assert.Equal(2, result[3].Line); // y (новая строка)
            Assert.Equal(2, result[3].Column); // Пробел перед y
        }
