...
namespace Parser.Parser
{
    public class Parser
    {
        ...
        // Список ошибок парсинга для диагностики
        ...
        private Expression ParsePrimary()
        {
            ...
            if (Match(TokenType.NUMBER, TokenType.FLOAT_LITERAL, TokenType.STRING_LITERAL, TokenType.CHAR_LITERAL, TokenType.BOOL_LITERAL))
            {
                var token = Previous();
                ...
                // Строки и символы
                return new LiteralExpression(token.Type == TokenType.STRING_LITERAL ? token.GetStringValue() : token.Text, token);
            }
        ...
        private Expression ParseNewExpression()
        {
            ...
            if (Match(TokenType.LPAREN))
            {
                ...
                {
                    do
                    { args.Add(ParseExpression()); } while (Match(TokenType.COMMA));
                }
                Consume(TokenType.RPAREN, "Expected ')' after constructor arguments.");
                return new NewObjectExpression(type, args);
            }

            var msg = "Expected array size or constructor call after 'new'.";
            Diagnostics.Add(new ParserDiagnostic(ParserDiagnosticLevel.Error, msg, Current));
            throw new ParserException(msg, Current);
        }
    }
